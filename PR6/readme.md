...
==8675==ABORTING
```
* glibc повністю довіряє коду та працює мит'єво та фіксує критичну помилку, конкретно структуру виділеної пам'яті що вийшла зі своїх меж, однак із запізненням при спробі виділити нову пам'ять. 
* Valgrind намагається відстежувати кожну операцію в емульованому процесорі, роблячі попередження вже на етапі виході зі меж робить попередження, однак все рівно пропускає незареєстровані записи та доводить прогарму до фаталу в наслідок виділенння нової пам'яті.
* AddressSanitizer завдяки shadow memory вже на другій ітерації заповнення помічає оверфлов та відразу зупиняє процес видаючи детальний трасувальник стеку з точним зазначенням винного рядка коду.
